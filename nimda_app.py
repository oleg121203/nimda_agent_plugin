    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            self.components = {
                ...
                "worker_agent": WorkerAgent(project_root),
            ...
            # Try to initialize individual components and report which ones fail
            component_status = {}

            try:
                from chat_agent import ChatAgent

                self.components["chat_agent"] = ChatAgent()
                component_status["chat_agent"] = "✅ OK"
            except Exception as comp_error:
                component_status["chat_agent"] = f"❌ {comp_error}"

            try:
                from worker_agent import WorkerAgent

                self.components["worker_agent"] = WorkerAgent(project_root)
                component_status["worker_agent"] = "✅ OK"
            except Exception as comp_error:
                component_status["worker_agent"] = f"❌ {comp_error}"

            try:
                from adaptive_thinker import AdaptiveThinker

                self.components["adaptive_thinker"] = AdaptiveThinker()
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["interactive", "health", "daemon"],
        default="interactive",
